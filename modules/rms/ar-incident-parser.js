...
(function (root, factory) {
    ...
    /* ----------------------------------------------------------------
     * Shared shape primitives (modules/rms/rms-shapes.js).
     *
     * The format-INDEPENDENT judgements — what a licence number, a street
     * address, a name or a paragraph of prose looks like — live in one place
     * so this reader and the generic (format-agnostic) reader can never drift
     * into disagreeing about the same token on the same page.
     *
     * Renderer: rms-shapes.js must be script-tagged BEFORE this file.
     * Node: resolved by require.
     * ---------------------------------------------------------------- */
    ...
    if (!S) {
        throw new Error('ar-incident-parser.js requires modules/rms/rms-shapes.js to be loaded first');
    }
    ...
    var RE_PAGE_HEADER = S.RE_PAGE_HEADER;
    var RE_INCIDENT_NO = /(?<!\d)(\d{2}-\d{6,8})(?!\d)/;
    var RE_ORI = /\b([A-Z]{2}\d{7})\b/;
    var RE_DATE = S.RE_DATE;
    /* The /g form is deliberately NOT shared. A global regex carries lastIndex
     * state, so handing the same instance to two readers lets one abandon a
     ...
     */
    var RE_DL_LABEL = S.RE_DL_LABEL;
    var RE_SSN_LABEL = S.RE_SSN_LABEL;
    var RE_DL_STATE_LABEL = S.RE_DL_STATE_LABEL;
    ...
    function _harvestIdentityRow(lines, labelIdx, to, person) {
        return S.harvestIdentityRow(lines, labelIdx, to, person);
    }
    ...
    /* ================================================================
     * Small helpers — delegated to rms-shapes.js so the generic reader and
     * this one can never disagree about the same token.
     * ================================================================ */

    function _lines(text) { return S.lines(text); }

    function _strip(line) { return S.strip(line); }

    function _clean(s) { return S.clean(s); }

    function _fmtPhone(line) { return S.fmtPhone(line); }

    function _isPageBreak(line) { return S.isPageBreak(line); }

    /* Prose test — used to keep narrative text and to keep label grids out of it. */
    function _isProse(line) { return S.isProse(line); }

    /* ---------------- name extraction ---------------- */

    function _extractName(line) { return S.extractName(line); }

    /* Juvenile / AKA alias printed inside the name field, kept as metadata only. */
    function _aliasOf(name) { return S.aliasOf(name); }
     ...
     * together: "1 164 S COKER RD, Vilonia, AR 72173". rms-shapes strips that
     * column only when doing so still leaves a street number behind.
     *
     * Skips the two rows that follow an ARREST LOCATION label — that address
     * is where the arrest happened, not where the person lives. */
    function _scanAddress(lines, from, to, blockEnd) {
        return S.scanAddress(lines, from, to, blockEnd);
    }
     ...
     *
     * The sentence-boundary regexes themselves now live in rms-shapes.js
     * (RE_SENTENCE_END / RE_ABBREV_END / RE_SENTENCE_START) so the generic
     * reader breaks paragraphs exactly the way this one does.
     * ---------------------------------------------------------------- */
    ...
    function _junkRatio(lines) {
        return S.junkRatio(lines);
    }

    function _paragraphize(body) {
        return S.paragraphize(body);
    }
    ...
    function _isNarrativeFragment(line) {
        return S.isNarrativeFragment(line);
    }
